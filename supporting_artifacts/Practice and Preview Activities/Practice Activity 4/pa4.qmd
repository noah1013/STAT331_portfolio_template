...
military_clean <- military |>
  mutate(across(`1988`:`2019`, \(x) na_if(x, ". ."))) |>
  mutate(across(`1988`:`2019`, \(x) na_if(x, "xxx")))
```
...
military_clean <- military_clean |>
  mutate(across(`1988`:`2019`, \(x) as.numeric(x)))
military_clean
```
...
The regions not removed from the military_clean dataset were Sub-Saharan, Central America and the Caribbean, Asia & Oceania, and the Middle East
...
long_military <- military_clean |>
  pivot_longer(cols = c(`1988`:`2019`), names_to = "Year", values_to = "Spending")
```
...
```{r question-5}
ggplot(data = long_military, aes(x = Year, y = Spending)) +
  geom_boxplot() + 
  guides(x = guide_axis(n.dodge = 3)) +
  labs(y = "Spending (% of total government spending)")
```
...
military_clean |>
  select(c(Country, `1991`)) |>
  slice_max(`1991`, n = 10)
```

The year 1990 has the second largest military expenditure, with Kuwait spending 52.56% of the total government spending on their military.

**Bonus**: What is the reason for this large expenditure?

The reason for this was because of the Iraqi invasion of Kuwait, on August 2nd, 1990. They held Kuwait for 7 months, which can be seen with its even larger spending in 1991.
